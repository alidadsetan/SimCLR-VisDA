...
if args.action == "pretrain":
    ...
    if args.pretrained_weights_path:
        model = SimCLR.load_from_checkpoint(Path(args.pretrained_weights_path).resolve())
    else:
    ...
    trainer = pl.Trainer(callbacks=callbacks, accelerator="dp", gpus=args.num_gpus, logger=[TensorBoardLogger(
