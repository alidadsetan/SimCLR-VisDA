...
class SSLOnlineEvaluator(Callback):  # pragma: no cover
    ...
    def __init__(
        ...
        first_ten_percent_length = len(train_dataset)//100
    ...
    def _compute_adaptation_loss(self, trainer: Trainer, pl_module: LightningModule, source: str, t_loader: DataLoader, epochs: int) -> None:
        ...
        for i in range(epochs):
            ...
            for batch in t_loader:
                ...
                print("linear_train_batch_acc", train_acc)
                # trainer.logger.experiment.log({
                #     "linear_train_batch_acc": train_acc,
                #     "batch": index,
                #     "epoch": i,
                # })
                if i == epochs - 1:
            ...
            if i == epochs - 1:
                epoch_train_acc = sum(
                    [x["acc"] * x["num"] for x in train_accs])/sum([x["num"] for x in train_accs])
                print("linear_train_acc", epoch_train_acc)
